<template>
  <div class="grid grid-cols-4 gap-4" v-if="$device.isDesktopOrTablet">
    <nuxt-link :to="{path: item.link, hash: item.hash}" v-for="item in items" :key="item.key"
               class="w-72 h-72 cursor-pointer relative"
  ...
  <div class="flex flex-row justify-center items-center w-full" v-else>
    <swiper :options="swiperOptions" v-if="items">
      <swiper-slide v-for="item in items" :key="item.key">
        <nuxt-link :to="{path: item.link, hash: item.hash}" class="relative flex items-center justify-center">
