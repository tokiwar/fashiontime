<template>
  <nav v-if="routes">
    <ul class="w-max flex flex-row space-x-4 text-2xl font-sans font-bold" itemscope itemtype="https://schema.org/BreadcrumbList">
      ...
      >
        ...
        <nuxt-link class="hover:underline" itemprop="item" :to="route.route"
                   v-if="route.route"
                   :class="{'underline pointer-events-none text-white' : route.current}">
          {{ route.name }}
        </nuxt-link>
